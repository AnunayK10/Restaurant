          ...
          {error ? (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {foodItems.map((item) => (
                <div
                  key={item._id}
                  className="relative flex flex-col text-gray-700 bg-white shadow-lg hover:shadow-xl transition-all duration-300 bg-clip-border rounded-xl w-full h-[300px] overflow-hidden min-w-[300px] min-h-[300px] hover:bg-gray-50"
                >
                  ...
                  <div className="p-3 bg-gradient-to-b from-white to-gray-50">
                    <div className="flex items-center justify-between mb-1">
                      <p className="block font-sans text-sm font-medium leading-relaxed text-gray-900">
                        {item.foodName}
                      </p>
                      <p className="block font-sans text-sm font-medium leading-relaxed text-green-600">
                        INR {item.price}
                      </p>
                    </div>
                    <p className="block font-sans text-xs font-normal leading-normal text-gray-600">
                      {item.description}
                    </p>
                  </div>
                  {currentUser?._id && (
                    <div className="absolute flex justify-between bottom-3 left-3 right-3">
                      <button
                        ...
                        className="px-4 py-2 font-semibold text-white rounded transform hover:scale-105 hover:shadow-lg active:scale-95 transition-all duration-300 ease-in-out"
